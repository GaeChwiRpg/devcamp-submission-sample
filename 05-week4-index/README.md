# 05-week4-index

> 이 미션은 강의 `6주 완성! 백엔드 이력서 차별화 전략 4가지` Week 3 (인덱스) 의 적용편입니다.

## 이번 주에 해야 하는 것

- 느린 조회 API 하나를 고릅니다.
- 왜 느린지 explain과 응답 시간으로 확인합니다.
- 인덱스를 추가한 뒤 before / after를 비교합니다.

## 이번 주에 제출할 것

- `report.md`
- `evidence/explain-before.md`
- `evidence/explain-after.md`
- `evidence/latency-comparison.md`
- `project/` 안 실제 코드

## 미션 시작 전에 보면 좋은 것

1. 이 README의 "이번 주에 해야 하는 것" + "평가 기준"
2. `project/README.md` — baseline + 미션 흐름 (단계별 가이드 포함)
3. `project/MEASUREMENT-OPTIONS.md` — 측정 도구 후보 메뉴 (EXPLAIN + 응답 시간 각 1택)
4. `evidence/README.md` 템플릿
5. `report.md` 템플릿
6. `docs/review-rubric.md`

(정답 PR은 미션 종료 후 운영진이 별도로 공유합니다.)

`project/` 안에 baseline 코드가 이미 들어 있어서 환경 세팅에 시간을 태우지 않습니다. 학생은 그 위에서 인덱스 미션만 다룹니다.

핵심은 "인덱스를 추가했다"가 아니라, **왜 추가했고 무엇이 좋아졌는지 비교로 증명하는 것**입니다. 측정 도구는 starter가 박지 않으니 직접 골라서 evidence에 선택 근거를 남기세요.
