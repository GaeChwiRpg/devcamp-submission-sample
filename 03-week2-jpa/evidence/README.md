...
이 폴더는 본 미션의 핵심(영속성 컨텍스트 + 연관관계 주인 + LAZY/N+1 + 변경 감지)이 본인 작업에 적용된 흔적을 남기는 곳입니다.

## 엔티티 / 연관관계 자산

- `entity-design-notes.md` — Post/Comment 엔티티 설계 + @Id 전략 결정
- `association-owner-decision.md` — 연관관계 주인 결정 근거

## 영속성 컨텍스트 4 기능 자산

- `n-plus-one-before.md` — Hibernate `show_sql` 출력 (LAZY 미적용 또는 N+1 발생 시점)
- `n-plus-one-after.md` — fetch join / `@EntityGraph` 적용 후 쿼리
- `dirty-checking-snapshot.md` — 변경 감지 활용 사례
